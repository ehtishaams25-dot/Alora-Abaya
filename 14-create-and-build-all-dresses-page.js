async function run() {
  console.log("=== CREATING AND BUILDING 'ALL DRESSES' COLLECTION PAGE ON A STRICTLY SEPARATE PAGE NODE ===");

  // 1. Get all pages and strictly identify the homepage so we NEVER touch it
  let pages = await framer.agent.getNodesOfTypes({ types: ["WebPageNode"] });
  ...
  // 2. Find any non-homepage that is /all-dresses or /shop, or create one if needed
  let targetPage = pages.find(p => p.id !== homePage.id && (p.attributes?.path === "/all-dresses" || p.attributes?.path === "/shop" || p.name === "All Dresses" || p.name === "Web Page"));

  if (!targetPage) {
    console.log("No separate /all-dresses or /shop page found. Creating a brand new WebPageNode...");
    await framer.agent.applyChanges('+WebPageNode AllDressesPage; SET AllDressesPage name="All Dresses" path="/all-dresses";', { pagePath: "/" });
    
    // Re-fetch pages to grab the newly created node
    pages = await framer.agent.getNodesOfTypes({ types: ["WebPageNode"] });
    targetPage = pages.find(p => p.id !== homePage.id && (p.attributes?.path === "/all-dresses" || p.name === "All Dresses"));
  }

  if (!targetPage) {
    throw new Error("Failed to find or create a non-homepage WebPageNode for All Dresses.");
  }

  // CRITICAL SAFETY CHECK
  if (targetPage.id === homePage.id || targetPage.attributes?.path === "/") {
    throw new Error("FATAL: Target page resolved to the homepage! Aborting to protect homepage.");
  }

  const pageId = targetPage.id;
  const bpId = targetPage.$breakpoints?.[0]?.id || targetPage.children?.[0]?.id;
  let targetPath = targetPage.attributes?.path;

  // If targetPath is not set, set it to /all-dresses
  if (!targetPath || targetPath === "") {
    console.log(`Setting path="/all-dresses" on page ${pageId}...`);
    // Note: when applying to a page whose path might still be undefined, we can pass pageId or any valid path
    await framer.agent.applyChanges(`SET ${pageId} name="All Dresses" path="/all-dresses";`, { pagePath: "/" });
    targetPath = "/all-dresses";
  } else if (targetPath !== "/all-dresses") {
    // Also ensure name is All Dresses
    await framer.agent.applyChanges(`SET ${pageId} name="All Dresses";`, { pagePath: targetPath });
  }

  console.log(`SAFE TARGET CONFIRMED -> Page ID: ${pageId}, Breakpoint ID: ${bpId}, Path: ${targetPath}`);
  ...
  // Setup canvas background & layout for All Dresses page breakpoint
  await applySafe(`SET ${bpId} width="1440px" height="auto" fill="#FAF9F6" layout="stack" stackDirection="vertical" gap="0px";`, targetPath, "Setup All Dresses Breakpoint");

  // Clean existing children of this specific breakpoint node only
  ...
  await applySafe(navDSL, targetPath, "Build AnnounceBar & NavBar on All Dresses");
  ...
  await applySafe(catalogContainerDSL, targetPath, "Build Sidebar Filters & Product Grid Rows");
  ...
  for (const item of products) {
    ...
    await applySafe(cardDSL, targetPath, `Build Product Card ${item.id}`);
  }

  // STEP 4: Build Exact Reference Editorial Footer at the bottom
  console.log("--- BUILDING EDITORIAL FOOTER ON ALL DRESSES PAGE ---");
  const footerDSL = [
  ...
  await applySafe(footerDSL, targetPath, "Build Reference Editorial Footer on All Dresses");

  console.log("=== ALL DRESSES COLLECTION PAGE COMPLETE ON SEPARATE PAGE NODE ===");
  return { status: "success", pagePath: targetPath, totalProducts: products.length };
}
