async function run() {
  console.log("=== BUILDING LUXURY CATALOG STRICTLY IN THE '/shop' PAGE NODE ===");
  ...
  // Find specifically the /shop page node
  const shopPage = pages.find(p => p.id !== homePage.id && (p.attributes?.path === "/shop" || p.name === "/shop" || p.name?.toLowerCase() === "shop"));
  
  if (!shopPage) {
    throw new Error("Could not find WebPageNode for '/shop'. Available pages: " + pages.map(p => `${p.name} (${p.attributes?.path})`).join(", "));
  }

  if (shopPage.id === homePage.id || shopPage.attributes?.path === "/") {
    throw new Error("FATAL: Target page resolved to homepage! Aborting to protect Home.");
  }
  ...
  // Setup /shop breakpoint canvas dimensions and styling
  ...
  await applySafe(catalogContainerDSL, targetPath, "Build Sidebar & Grid on /shop");

  // STEP 3: Build 8 Luxury Product Cards
  const products = [
    {
      id: "P1",
      row: "GridRow1",
  ...
  // STEP 4: Build Editorial Footer on /shop
  const footerDSL = [
  ...
  console.log("=== LUXURY CATALOG SUCCESSFULLY BUILT IN '/shop' ONLY ===");
